...
class ILQRSolverParameters:
    ...
    def __post_init__(self) -> None:
        ...
        assert all(len(pos) == 2 for pos in self.obstacle_positions), "Obstacle positions must be 2D coordinates"
        if len(self.obstacle_positions) > 0:
            assert self.obstacle_safety_distance > 0.0, "Safety distance must be positive when obstacles exist"
            assert self.obstacle_cost_weight > 0.0, "Obstacle cost weight must be positive when obstacles exist"
        else:
            assert self.obstacle_safety_distance >= 0.0, "Safety distance cannot be negative"
            assert self.obstacle_cost_weight >= 0.0, "Obstacle cost weight cannot be negative"
...
class ILQRSolver:
    ...
    def _compute_tracking_cost(self, iterate: ILQRIterate, reference_trajectory: DoubleMatrix) -> float:
        ...
        # Calculate costs
        input_cost = np.sum([u.T @ self._input_cost_matrix @ u for u in input_trajectory])
        state_cost = np.sum([e.T @ self._state_cost_matrix @ e for e in error_state_trajectory])
        obstacle_cost = self._compute_obs_avoidance_cost(state_trajectory)
        
        cost = input_cost + state_cost + obstacle_cost

        return float(cost)

    def _compute_obs_avoidance_cost(self, state_trajectory: np.ndarray) -> float:
        """
        Compute obstacle avoidance cost for the given state trajectory.
        
        Args:
            state_trajectory: Array of shape (N, 5) containing full state vectors
            
        Returns:
            Total obstacle avoidance cost
        """
        if not self._solver_params.obstacle_positions:
            return 0.0
            
        cost = 0.0
        safe_dist = self._solver_params.obstacle_safety_distance
        weight = self._solver_params.obstacle_cost_weight
        
        for state in state_trajectory:
            x, y = state[0], state[1]
            for obs_x, obs_y in self._solver_params.obstacle_positions:
                dx = x - obs_x
                dy = y - obs_y
                distance = np.sqrt(dx**2 + dy**2 + 1e-6)  # Add small epsilon to avoid division by zero
                
                if distance < safe_dist:
                    # Quadratic penalty for safety margin violation
                    cost += weight * (safe_dist - distance)**2
    ...
    def _run_lqr_backward_recursion(
        ...
        for i in reversed(range(N)):
            ...
            # Calculate obstacle cost gradient and hessian relative to reference
            obstacle_grad = np.zeros(self._n_states)
            obstacle_hessian = np.zeros((self._n_states, self._n_states))
            
            if len(self._solver_params.obstacle_positions) > 0:
                # Get current state (reference + error)
                current_x = reference_trajectory[i, 0] + error[0]
                current_y = reference_trajectory[i, 1] + error[1]

                # Calculate obstacle cost derivatives for each obstacle
                for (obs_x, obs_y) in self._solver_params.obstacle_positions:
                    ...
                    distance_sq = dx**2 + dy**2 + 1e-6  # avoid division by zero
                    distance = np.sqrt(distance_sq)
                    safe_dist = self._solver_params.obstacle_safety_distance
                    dist_to_safe = distance - safe_dist
                    
                    if dist_to_safe < 0:
                        # Use quadratic penalty when inside safety margin
                        penalty = self._solver_params.obstacle_cost_weight * dist_to_safe**2
                        
                        # First derivatives
                        dpenalty_dx = 2 * self._solver_params.obstacle_cost_weight * dist_to_safe * dx / distance
                        dpenalty_dy = 2 * self._solver_params.obstacle_cost_weight * dist_to_safe * dy / distance
                        
                        # Second derivatives
                        d2penalty_dx2 = 2 * self._solver_params.obstacle_cost_weight * (
                            (dx**2 * (1 - safe_dist/distance) + safe_dist * dy**2 / distance) / distance_sq
                        )
                        d2penalty_dy2 = 2 * self._solver_params.obstacle_cost_weight * (
                            (dy**2 * (1 - safe_dist/distance) + safe_dist * dx**2 / distance) / distance_sq
                        )
                        d2penalty_dxdy = 2 * self._solver_params.obstacle_cost_weight * (
                            dx * dy * (1 - safe_dist/distance - safe_dist/(2*distance)) / distance_sq
                        )

                        # Accumulate gradients and hessians
                        obstacle_grad[0] += dpenalty_dx
                        obstacle_grad[1] += dpenalty_dy
                        
                        obstacle_hessian[0, 0] += d2penalty_dx2
                        obstacle_hessian[1, 1] += d2penalty_dy2 
                        obstacle_hessian[0, 1] += d2penalty_dxdy
                        obstacle_hessian[1, 0] += d2penalty_dxdy  # Hessian is symmetric

            # Update P matrix and rho vector with obstacle terms (already scaled by weight)
            p_prior = (
                ...
                + (obstacle_hessian if len(self._solver_params.obstacle_positions) > 0 else 0)  # Add obstacle Hessian
            )
            ...
            rho_prior = (
                ...
                + (obstacle_grad if len(self._solver_params.obstacle_positions) > 0 else 0)  # Add obstacle gradient
            )
