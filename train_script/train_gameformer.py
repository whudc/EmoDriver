...
# from gameformer.pluto import PlutoPlanner
from torch.utils.data import DataLoader
from gameformer.train_utils import *
from torch.utils.tensorboard import SummaryWriter
torch.multiprocessing.set_sharing_strategy('file_system')

def train_epoch(data_loader, model, optimizer, epoch_id, writer):
    epoch_loss = []
    epoch_metrics = []
    model.train()
    ...
    with tqdm(data_loader, desc="Training", unit="batch") as data_epoch:
        for batch in data_epoch:
            ...
            writer.add_scalar('train/total_loss', loss.item(), epoch_id * len(data_loader) + len(epoch_loss))
...
def model_training():
    ...
    writer_tb = SummaryWriter(log_path)
    ...
    # set up model
    if args.model_name == "gameformer":
        gameformer = GameFormer(encoder_layers=args.encoder_layers, decoder_levels=args.decoder_levels, neighbors=args.num_neighbors)
    else:
        gameformer = PlutoPlanner()
    # pretrain_weights = torch.load('./gameformer_challenge/predictor.pth', map_location=torch.device('cpu'))
    # gameformer.load_state_dict(pretrain_weights, strict=False)
    ...
    if args.resume is not None and os.path.exists(args.resume):
        ...
        gameformer.load_state_dict(checkpoint["model_state"])
    ...
    # set up data loaders
    if args.train_set.endswith('.json'):
        train_set = DrivingData(args.train_set, args.num_neighbors)
        valid_set = DrivingData(args.valid_set, args.num_neighbors)
    else:
    ...
    for epoch in range(start_epoch, train_epochs):
        ...
        train_loss, train_metrics = train_epoch(train_loader, gameformer, optimizer, epoch, writer_tb)
        val_loss, val_metrics = valid_epoch(valid_loader, gameformer)
        writer_tb.add_scalar('val/loss', val_loss, epoch)
        ...
        checkpoint = {
            "epoch": epoch + 1,
            "model_state": gameformer.state_dict(),
            "optimizer_state": optimizer.state_dict(),
            "scheduler_state": scheduler.state_dict(),
        }
        torch.save(checkpoint, f"{log_path}/model_epoch_{epoch+1}_valADE_{val_metrics[0]:.4f}.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', type=str, help='log name (default: "Exp1")', default="Exp_ckpt")
    parser.add_argument('--train_set', type=str, default=None, help='path to train data')
    parser.add_argument('--valid_set', type=str, default=None, help='path to validation data')
    ...
    parser.add_argument('--learning_rate', type=float, help='learning rate (default: 1e-4)', default=2e-4)
    parser.add_argument('--device', type=str, help='run on which device (default: cuda)', default='cuda')
    parser.add_argument('--model_name', type=str, help='pretrained model name', default="gameformer")
