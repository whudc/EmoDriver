...
class SelfTransformer(nn.Module):
    ...
    def forward(self, inputs, mask=None, llm_feature=None):
        if mask.dtype == torch.bool:
            ...
            float_mask = torch.where(mask, 
                                    torch.tensor(float('-inf'), device=mask.device, dtype=inputs.dtype),
                                    torch.tensor(0.0, device=mask.device, dtype=inputs.dtype))
            # MHA 的 API 通常要求 float mask 的形状是 (N, L) 或 (L, N)
            # 根据你使用的 MHA API，你可能需要调整 float_mask 的形状
            
        else:
            float_mask = mask # 如果已经是 float mask，则保持
